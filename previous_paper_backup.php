...
<?php
 ...
 -->
      	...
      	//getting long_ques start

      	$long_ques_query="SELECT `question` FROM `long_question` WHERE `ques_id`='$long_ques_no'";
      	$run_long_query=mysqli_query($conn,$long_ques_query);

      	while($long_ques_data=mysqli_fetch_assoc($run_long_query))
      	{
      		$count_long++;
      		$long_q=$long_ques_data['question'];
      		?>
      			<p class="margin_bottom_0px"><?php echo $count_long." ".$long_q; ?></p>
      		<?php
      		
      	}

      	//getting long_ques end

      	 //getting short_ques start
      	$short_ques_query="SELECT `question` FROM `short_question` WHERE `ques_id`='$short_ques_no'";
      	$run_short_query=mysqli_query($conn,$short_ques_query);

      	while($short_ques_data=mysqli_fetch_assoc($run_short_query))
      	{
      		$count_short++;
      		$short_q=$short_ques_data['question'];
      		?>
      			<p class="margin_bottom_0px"><?php echo $count_short." ".$short_q; ?></p>
      		<?php
      		
      	}

      	//getting short_ques end
